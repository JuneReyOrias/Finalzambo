    ...
    <div class="card border-0 shadow rounded">
        ...
        height: 90px;">
                        ...
                        <div class="profile-head text-center mt-3"> <!-- Centered the profile head -->
                            ...
                            <h5 class="h5 profile-rating">Name: <span>{{ !empty($data['full_name']) ? ucwords(strtolower(trim($data['full_name']))) : 'N/A' }}</span></h5>
                        ...
                        <div class="collapse" id="skillsDetails">
                            <div class="card mt-2">
                                <div class="card-body">
                                    ...
                                    @foreach ($cropsData as $crop)
                                    @foreach ($crop['lastProductionDatas'] as $lastProductionData)
                        ...
                        <div class="collapse" id="projectsDetails">
                            <div class="card mt-2">
                                ...
                                <p>
                                    ...
                                    <span class="highlight-target">{{ ucwords(strtolower($gpsData['tenurial_status'])) }}</span>
                                </p>
                                <p>
                                <strong>Farm Address:</strong>
                                <span class="highlight-target">
                                    {{ !empty($gpsData['FarmAddress']) ? ucwords(strtolower($gpsData['FarmAddress'])) : 'N/A' }}
                                </span>
                                </p>
                                <p>
                                <strong>No. of years as Farmer:</strong>
                                <span class="highlight-target">{{ number_format($gpsData['NoYears'], 1) }}</span>
                                </p>
                                <p>
                                    <strong>Total Physical Area:</strong>
                                    <span class="highlight-target">{{ number_format($gpsData['totalPhysicalArea'], 1) }}</span>
                                </p>
                                <p>
                                    <strong>Total Area Cultivated:</strong>
                                    <span class="highlight-target">{{ number_format($gpsData['TotalCultivated'], 1) }}</span>
                                </p>
                                <p>
                                <strong>Area Prone To:</strong>
                                <span class="highlight-target">{{ ucwords(strtolower($gpsData['area_prone_to'])) }}</span>
                                </p>
                                <p>
                                <strong>Ecosystem:</strong>
                                <span class="highlight-target">{{ ucwords(strtolower($gpsData['ecosystem'])) }}</span>
                                </p>
                                ...
                                <p>
                                    ...
                                    <span class="highlight-target">{{ $gpsData['rsba_registered'] ? 'Yes' : 'No' }}</span>
                                </p>
                                <p>
                                    <strong>PCIC Insured:</strong>
                                    <span class="highlight-target">{{ $gpsData['pcic_insured'] ? 'Yes' : 'No' }}</span>
                                </p>
                                <p>
                                    <strong>Government Assisted:</strong>
                                    <span class="highlight-target">{{ $gpsData['government_assisted'] ? 'Yes' : 'No' }}</span>
                                </p>
                                <p>
                                    <strong>Source Capital:</strong>
                                    <span class="highlight-target">{{ ucwords(strtolower($gpsData['source_of_capital'])) }}</span>
                                </p>
                                ...
                                <p>
                                    ...
                                    <span class="highlight-target">{{ ucwords(strtolower($crop['cropName'])) }}</span>
                                </p>
                                <p>
                                <strong>Crop Variety Planted:</strong>
                                <span class="highlight-target">
                                    {{ !empty($crop['cropVariety']) ? ucwords(strtolower($crop['typeOfVariety'])) : 'N/A' }}
                                </span>
                                </p>
                                <p>
                                <strong>No. of Cropping/Year:</strong>
                                <span class="highlight-target">{{ number_format($crop['croppingNumberperyear']) }}</span>
                                </p>
                                <p>
                                <strong>Planting Schedule (Wet Season):</strong>
                                <span class="highlight-target">{{ (($crop['plantingScheduleWet'])) }}</span>
                                </p>
                                <p>
                                    <strong>Planting Schedule (Dry Season):</strong>
                                    <span class="highlight-target">{{ (($crop['plantingScheduleDry'])) }}</span>
                                </p>
                                ...
                                <p>
                                    ...
                                    <span class="highlight-target">{{ number_format($crop['yield'], 2) }}</span>
                                </p>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mt-1 mb-3">
                                            @foreach ($cropsData as $crop)
                                    @foreach ($crop['variableCosts'] as $variableCostsData)
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="collapse" id="fixedCostDetails">
                            <div class="card mt-2">
                                <div class="card-body">
                                    @foreach ($cropsData as $crop)
                                    @foreach ($crop['fixedCosts'] as $fixedCostsData)
                                    <h6 class="mt-2 highlight"> FixedCost</h6>
                                    <p>
                        ...
                        <div class="collapse" id="machineriesDetails">
                            <div class="card mt-2">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mt-1 mb-3">
                                        @foreach ($cropsData as $crop)
                                        @foreach ($crop['machineries'] as $machineriesData)
                                        ...
                                        <div class="col-md-6 mt-2">
                                            ...
                                            @foreach ($crop['machineries'] as $machineriesData)
                                                        ...
                                                        <p>
                                                            ...
                                                            <h6 class="mt-2 highlight">Total Cost For Machineries</h6>
                                                            <span class="highlight-target">{{ number_format($machineriesData->total_cost_for_machineries,2) }}</span>
                                                        </p>
                               ...
                               <div class="row mr-4 mb-4">
                                <div class="col-md-12 d-flex justify-content-between align-items-center mb-3">
                                    <div class="text-start ml-3">
                                        <a href="{{ route('agent.FarmerInfo.farm_view', $farmProfile->personal_informations_id) }}" class="btn btn-primary" onclick="goBack()">Back</a>
                                    </div>
                                    <div class="text-end">
            ...
            <div class="modal-body" id="modalContentToPrint">
                <div class="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <div class="right-logo mr-5">
                      ...
                      @else
                        <img src="../assets/logo/Citylogo.jpg" alt="Profile Image" class="img-fluid" style="max-height: 90px; max-width:80">
                      @endif
                    </div>
                    ...
                    <div class="left-logo  ml-5">
                      ...
                      @else
                        <img src="../assets/logo/agriculture.jpg" alt="Profile Image" class="img-fluid" style="max-height: 90px; max-width:80">
                      @endif
                    </div>
                </div>
                   ...
                   <div class="profile-head text-center mt-5">
                    <h5 class="h5 profile-rating highlight-target">Name: <span>{{ !empty($data['full_name']) ? ucwords(strtolower(trim($data['full_name']))) : 'N/A' }}</span></h5>

                            <h5  class="h5 rofile-rating highlight-target">Age: <span>{{ $data['age'] }}</span></p> <!-- Age -->
                </div>
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-hover-color table-bordered align-middle">
                        ...
                        <tbody>
                            ...
                            @foreach ($cropsData as $crop)
                            @foreach ($crop['lastProductionDatas'] as $productionData)
                                    @php
                                        ...
                                        $cropName = $crop['cropName']; // Get the crop name for each crop farm
...
$(document).ready(function() {
    ...
    const farmProfile = {{ $farmProfile->id }}; // Assuming you have a farm data object passed to the view
    ...
    $.ajax({
        url: `/agent-view-farmer-profile/${farmProfile}`, // Use the farmData ID in the URL
        method: 'GET',
...
$(document).ready(function() {
    // Get the authenticated user ID and farm profile ID dynamically
    const userId = {{ auth()->user()->id }}; // Ensure user ID is available
    const farmProfile = {{ $farmProfile->id }}; // Farm profile ID

    // Fetch the data using jQuery AJAX
    $.ajax({
        url: `/agent-view-farmer-profile/${farmProfile}`, // Update with your actual route
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            // Log userId and farmProfile to the console (optional)
            console.log('User ID:', userId);
            console.log('Farm Profile ID:', farmProfile);
            
            // Prepare options for the ApexCharts bar chart for cropping data
            var options = {
                chart: {
                    type: 'bar',
                    height: 350
                ...
                series: [{
                    ...
                    data: data.formattedCroppingData.data // Yield data in tons
                }],
                xaxis: {
                    categories: data.formattedCroppingData.labels, // Cropping numbers
                    title: {
...
$(document).ready(function() {
    ...
    $.ajax({
        url: `/agent-view-farmer-profile/${farmProfile}`, // Update with your actual route URL using farmProfile ID
        method: 'GET',
        success: function(response) {
            // Check if data is returned successfully
            if (response && response.formattedYieldData) {
                const cropData = response.formattedYieldData;
...
$(document).ready(function() {
    $('#cropNameFilter').on('change', function() {
        ...
        $.ajax({
            url: '/admin-view-farmer-profile/{farmData}', // Laravel route
            method: 'POST',
            data: {
