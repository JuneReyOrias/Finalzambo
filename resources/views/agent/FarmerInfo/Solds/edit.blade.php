...
<script>
    $(document).ready(function() {
        $('#successModal').modal('show'); // Automatically show the modal when the session message is present
    });
</script>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <!-- Redirect Button -->
        <a href="/agent-view-farmer-production/{{ $cropData->id }}" class="btn btn-primary">Go to Production View</a>
      </div>
    </div>
  </div>
</div>
@endif
                                                ...
                                                <div class="content">
                                    <form action{{'UpdateSolds'}} method="POST">
                                        @csrf
            ...
            <div id="collapseFarmProfile" class="collapse show" aria-labelledby="headingFarmProfile" data-parent="#accordionFarmProfile">
                <div class="card-body">
                        ...
                        <!-- Crop Fields -->
                          ...
                          <input type="hidden" class="form-control users_id" placeholder="no of cropping/year" name="last_production_datas_id" value="{{$Solds->last_production_datas_id}}" id="no_of_cropping_per_year">
                            ...
                            <div class="user-details">
                                ...
                                <div class="input-box col-md-4">
                                  ...
                                  <select class="form-control crops_farms_id" name="crops_farms_id" id="crop_name">
                                    <option value="{{$Solds->crops_farms_id}}">{{$Solds->cropFarm->crop_name}}</option>
                         ...
                         <div class="input-box col-md-4">
                            ...
                            <input type="hidden" class="form-control light-gray-placeholder sold_to"value="{{$Solds->sold_to}}" name="sold_to" id="sold_to" placeholder="Enter sold to">
                        </div>
                       
                        <div class="input-box col-md-4">
                            <span class="details">Measurement/unit:</span>
                            <div class="d-flex align-items-center">
                                <select class="form-control custom-select light-gray-placeholder unit placeholder-text @error('unit') is-invalid @enderror" id="unit" name="measurement" onchange="Unit()">
                                    <option value="{{$Solds->measurement}}">{{$Solds->measurement}}</option>
                                    <option value="tons" {{ old('measurement') == 'tons' ? 'selected' : '' }}>tons</option>
                                    <option value="kg" {{ old('measurement') == 'kg' ? 'selected' : '' }}>kg</option>
                                    <option value="Add" {{ old('measurement') == 'Add' ? 'selected' : '' }}>Add</option>
                                </select>
                        ...
                        <div class="input-box col-md-4">
                            <span class="details">Unit Price/kg:</span>
                            <input type="text" class="form-control light-gray-placeholder unit_price_sold" 
                                   value="{{number_format($Solds->unit_price_rice_per_kg,2)}}" 
                                   name="unit_price_rice_per_kg" 
                                   id="unit_price_rice_per_kg" 
                                   placeholder="Enter unit price">
                        </div>
                        <div class="input-box col-md-4">
                            <span class="details">Quantity:</span>
                            <input type="text" class="form-control light-gray-placeholder quantity" 
                                   value="{{number_format($Solds->quantity,2)}}" 
                                   name="quantity" 
                                   id="quantity" 
                        ...
                        <div class="input-box col-md-4">
                            ...
                            <input type="text" class="form-control light-gray-placeholder gross_income" 
                                   value="{{$Solds->gross_income,}}" 
                                   name="gross_income" 
                                   id="gross_income" 
                                   placeholder="Enter gross income" readonly>
                        </div>
...
document.getElementById('unit_price_rice_per_kg').addEventListener('input', calculateGrossIncome);
    ...
    document.getElementById('unit').addEventListener('change', calculateGrossIncome);
    ...
    function calculateGrossIncome() {
        ...
        // Convert tons to kilograms if needed
        if (unit === 'tons') {
